...
parser.add_argument('--drop_rate_input', type=float, default=0.1, help='Drop out rate for input layer[default: 0.1]')
parser.add_argument('--drop_rate_hidden', type=float, default=0, help='Drop out rate for hidden layer [default: 0.5]')
...
mid_neuron_num_1 = 40
dp_keep_prob_input = 1 - FLAGS.drop_rate_input
dp_keep_prob_hidden = 1 - FLAGS.drop_rate_hidden
write_result = True
weight_decay_conv = FLAGS.weight_decay
weight_decay_fc = FLAGS.weight_decay
l1_value = 0.000
use_bn = True
...
def wave_block(input, len_input, num_outputs, is_training, scope, l1_value, dp_kp):
    with tf.variable_scope(scope):
        lp_coe, hp_coe, all_coe = wave_op(input, len_input, scope='wave_func',
                                          is_training=is_training, l1_value=l1_value, weight_decay=weight_decay_fc,
                                          sim_reg=SIM_REG)

        mid_layer_1 = fully_connected(all_coe, mid_neuron_num_1, bn=use_bn,
                                      is_training=is_training, weigth_decay=weight_decay_fc, scope='ran_min_1',
                                      bn_decay=0.0)
        ...
        predict = fully_connected(mid_layer_1, num_outputs, bn=use_bn, activation_fn=None,
                                  is_training=is_training, weigth_decay=weight_decay_fc, scope='ran_pred_1',
                                  bn_decay=0.0)
...
def wave_block_res(input, len_input, num_outputs, is_training, scope, l1_value, dp_kp):
    with tf.variable_scope(scope):
        ...
        conv1 = res_block(all_coe, 8, 8, scope='conv1_1', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv1 = res_block(conv1, 8, 8, scope='conv1_2', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv1 = res_block(conv1, 8, 8, scope='conv1_3', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv1 = max_pool2d(conv1, [2, 1], scope='pool1', stride=[2, 1])

        conv2 = res_block(conv1, 16, 5, scope='conv2_1', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv2 = res_block(conv2, 16, 5, scope='conv2_2', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv2 = res_block(conv2, 16, 5, scope='conv2_3', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv2 = max_pool2d(conv2, [2, 1], scope='pool2', stride=[2, 1])

        conv3 = res_block(conv2, 32, 3, scope='conv3_1', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv3 = res_block(conv3, 32, 3, scope='conv3_2', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
        conv3 = res_block(conv3, 32, 3, scope='conv3_3', bn=use_bn, weight_decay=weight_decay_conv,
                          is_training=is_training)
...
def get_model_conv(input, len_input, num_outputs, is_training, level_wave=3):
    lp_1, predict_1 = wave_block_conv(input, len_input, num_outputs, is_training,
                                      scope='wave_level_1', l1_value=l1_value, dp_kp=dp_keep_prob_input)

    lp_2, predict_2 = wave_block_conv(lp_1, len_input / 2, num_outputs, is_training,
                                      scope='wave_level_2', l1_value=l1_value, dp_kp=dp_keep_prob_hidden)

    predict_2 = tf.add(predict_2, predict_1, name='adding_2')

    lp_3, predict_3 = wave_block_conv(lp_2, len_input / 4, num_outputs, is_training,
                                      scope='wave_level_3', l1_value=l1_value, dp_kp=dp_keep_prob_hidden)

    predict_3 = tf.add(predict_3, predict_2, name='adding_3')

    lp_4, predict_4 = wave_block_conv(lp_3, len_input / 8, num_outputs, is_training,
                                      scope='wave_level_4', l1_value=l1_value, dp_kp=dp_keep_prob_hidden)
...
def get_model_res(input, len_input, num_outputs, is_training, level_wave=3):
    ...
    lp_2, predict_2 = wave_block_res(lp_1, len_input / 2, num_outputs, is_training,
                                     scope='wave_level_2', l1_value=l1_value, dp_kp=dp_keep_prob_hidden-0.1)
    predict_2 = tf.add(predict_2, predict_1, name='adding_2')

    lp_3, predict_3 = wave_block_res(lp_2, len_input / 4, num_outputs, is_training,
                                     scope='wave_level_3', l1_value=l1_value, dp_kp=dp_keep_prob_hidden-0.2)
    predict_3 = tf.add(predict_3, predict_2, name='adding_3')

    lp_4, predict_4 = wave_block_res(lp_3, len_input / 8, num_outputs, is_training,
                                     scope='wave_level_4', l1_value=l1_value, dp_kp=dp_keep_prob_hidden-0.3)
    predict_4 = tf.add(predict_4, predict_3, name='adding_4')
    ...
    with tf.Graph().as_default():
        ...
        for epoch_idx in range(MAX_EPOCH):
            ...
            if epoch_idx % 5 == 0:
                log_string("--------------------------------")
                log_string(
                    "Test loss for epoch %d: %f, Acc is %f, Max Acc is %f" % (epoch_idx, loss_mean, acc_mean, max_acc))
